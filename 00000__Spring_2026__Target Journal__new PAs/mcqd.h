...
// ===== ADDED (checkpoint save of permutations) =====
#include <fstream>
#include <string>
#include <vector>
...
class Maxclique {
  ...
  // ===== ADDED: pointer to permutation lines (1-based) =====
  const std::vector<std::string>* permLines1based;
public:
  void set_perm_lines(const std::vector<std::string>* p) { permLines1based = p; }
private:
  // ========================================================
  ...
  // ===== ADDED: Save best clique permutations when it improves =====
  void save_checkpoint_perms() const {
    const char* tmp = "best_clique_perms.tmp";
    const char* out = "best_clique_perms.txt";

    std::ofstream f(tmp);
    if (!f.is_open()) return;

    f << "# best clique so far (permutations)\n";
    f << "# size = " << QMAX.size() << "\n";
    f << "# format: serial. vertex_id permutation\n\n";

    for (int i = 0; i < QMAX.size(); i++) {
      int vid = QMAX.at(i);
      std::string perm = "";
      if (permLines1based && vid >= 0 && vid < (int)permLines1based->size()) {
        perm = (*permLines1based)[vid];
      }
      f << (i + 1) << ". " << vid << "  " << perm << "\n";
    }
    f.close();
...
void Maxclique::_mcq(int* &maxclique, int &sz, bool dyn) { 
  ...
  for (int i=0; i<QMAX.size(); i++) maxclique[i] = QMAX.at(i);
  sz = QMAX.size();
}
...
void Maxclique::Vertices::init_colors() { 
  ...
  for (int i = max_degree; i < sz; i++) v[i].set_degree(max_degree + 1);
}
  ...
  while (R.size()) {
    if (Q.size() + R.end().get_degree() > QMAX.size()) {
      ...
      if (Rp.size()) {
        ...
        QMAX = Q;
        save_checkpoint_perms();   // SAVE PERMS ON IMPROVEMENT
      }    
      Rp.dispose();
      Q.pop();
    } else return;
...
void Maxclique::expand_dyn(Vertices R) {
  ...
  while (R.size()) {
    if (Q.size() + R.end().get_degree() > QMAX.size()) {
      ...
      cut2(R, Rp);
      if (Rp.size()) {
        if ((float)S[level].get_i1()/++pk < Tlimit) degree_sort(Rp);
        color_sort(Rp);
        S[level].inc_i1();
        level++;
        expand_dyn(Rp);
